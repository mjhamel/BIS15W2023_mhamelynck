...
hummingbirds_matrix <- matrix(hummingbirds, nrow=3, byrow = F)
hummingbirds_matrix
```
...
dataframe \<- rbind(dataframe, new vector)
...
1.  Build a data frame called `hsprings` with the above data. Name the temperature column `temp_C`. Print out the data frame.
    ...
    new object as data frame -\> data.frame(objects with data in direction of columns/data going vertically)\
...
2.  Change the column titled `name` to `scientist`, leave the other column names the same. Print out the data frame names.
    ...
    new_name = old name within data.frame()\
...
4.  Calculate the mean temperature of all of the temperature measurements.
    ...
    new object -\> dataframe\$column

```{r}
mtemp <- hsprings$temp_c
mean(mtemp)
```
