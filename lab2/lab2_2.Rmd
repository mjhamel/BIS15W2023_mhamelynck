...
my_vector_sequence == 15
my_vector_sequence[my_vector_sequence==14]
...
1.  What are the total earnings for the US and non-US regions?
    ...
    ??any way to print only the global sum
...
```{r}
totals <- colSums(all_harry_potter_matrix)
totalshpm <- rbind(all_harry_potter_matrix, totals)
totalshpm
...
??can i select multiple rows in a specified column (pulling elements out of vectors/matrices)

Adding a colon `:` selects the specified elements in a column.

```{r}

secondcolumn <- harry_potter_matrix[,2]
secondcolumn
...
non_us_earningstotal <- all_harry_potter_matrix[ ,2]
```
